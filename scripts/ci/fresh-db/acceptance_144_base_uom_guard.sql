...
INSERT INTO public.products
  ...
  ('d0000000-0000-0000-0000-000000000005', '11111111-1111-1111-1111-111111111111', 'PM2', 'Mapped reconciliation', 'pcs',
   '40000000-0000-0000-0000-000000000001', 'MAPPED'),
  ('d0000000-0000-0000-0000-000000000006', '11111111-1111-1111-1111-111111111111', 'PU6', 'Unset 6', 'box', NULL, 'NO_UNIT'),
  ('d0000000-0000-0000-0000-000000000007', '11111111-1111-1111-1111-111111111111', 'PM3', 'Mapped source resolution', 'pcs',
   '40000000-0000-0000-0000-000000000001', 'MAPPED');
...
INSERT INTO public.uom_backfill_issues
  ...
  ('f0000000-0000-0000-0000-000000000008', '11111111-1111-1111-1111-111111111111', 'products', 'd0000000-0000-0000-0000-000000000007', 'UNIT_AMBIGUOUS_OR_UNKNOWN', 'OPEN');
...
-- Idempotent same-unit reconciliation succeeds and closes stale open issues.
SELECT public.rpc_assign_product_base_uom(
  '11111111-1111-1111-1111-111111111111',
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'ACCEPTANCE_FAIL: same-unit reconciliation did not close stale issue';
  END IF;
END $$;

-- Reinterpretation is rejected for mapped products, with or without SLE.
...
SELECT pg_temp.expect_error(
  $$ SELECT public.rpc_resolve_uom_backfill_issue('11111111-1111-1111-1111-111111111111','f0000000-0000-0000-0000-000000000008','40000000-0000-0000-0000-000000000002',NULL) $$,
  'UOM_BACKFILL_RESOLUTION_UOM_MISMATCH');
SELECT public.rpc_resolve_uom_backfill_issue(
  '11111111-1111-1111-1111-111111111111',
