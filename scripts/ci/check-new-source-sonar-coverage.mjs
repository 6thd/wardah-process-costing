...
// Pure, no I/O: glob matching for sonar.exclusions / sonar.coverage.exclusions,
// delegated entirely to Node's own built-in path.matchesGlob() (stable since
// Node 22.5, and this repo's Sonar workflow already pins Node 24) instead of
// a hand-rolled `new RegExp()` translation. Verified directly against every
// real pattern in sonar-project.properties (`**/dir/**`, `**/*.test.tsx`,
// `sql/migrations/00_*.sql`, `e2e/**`, `**/node_modules/**`) — all match the
// same files this gate previously computed by hand.
...
function main() {
  // Literal path arguments on every filesystem call below, deliberately not
  // built from a variable (e.g. via path.join(repoRoot, ...)): this script
  // only ever reads these two fixed, repo-relative files, resolved against
  // process.cwd() the same way `git`'s cwd below is — there is no path to
  // parametrize, so none is exposed.
  ...
  const sonarConfig = parseSonarProperties(fs.readFileSync('sonar-project.properties', 'utf8'));
  const lcovSourceFiles = parseLcovSourceFiles(fs.readFileSync('coverage/lcov.info', 'utf8'));
