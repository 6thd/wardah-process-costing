...
// scripts/ci/fresh-db/acceptance_171_ai_usage_daily.sql — there is nothing
// concurrent to exercise at this layer, since each Edge Function invocation
// is an independent request that defers all shared-state arbitration to
// that single RPC call.
...
Deno.test('buildSystemPrompt and buildUserPrompt never mention the quota constants', () => {
  ...
  const user = buildUserPrompt({ operation: 'summary', locale: 'en', data: { totalSales: 5 } })
  assertEquals(sys.includes(String(USER_DAILY_LIMIT)), false)
  assertEquals(user.includes(String(ORG_DAILY_LIMIT)), false)
})
...
Deno.test('quota RPC is called only through the admin client, with internal (not caller-supplied) limits', async () => {
  ...
  // Even though the body carries no limit fields at all (validateBody
  // strips anything not in its schema), assert the RPC still receives the
  ...
  assertEquals(capturedParams?.p_user_daily_limit, USER_DAILY_LIMIT)
  assertEquals(capturedParams?.p_org_daily_limit, ORG_DAILY_LIMIT)
})
