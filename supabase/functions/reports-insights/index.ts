...
// Internal, non-negotiable quota constants. Never accepted from the caller —
// a client-supplied limit would let any caller raise its own ceiling.
export const USER_DAILY_LIMIT = 20
export const ORG_DAILY_LIMIT = 100
...
export async function handleRequest(req: Request, deps: HandleRequestDeps = defaultDeps): Promise<Response> {
  ...
  const { data: quotaRows, error: quotaError } = await adminClient.rpc('rpc_check_and_record_ai_usage', {
    ...
    p_org_daily_limit: ORG_DAILY_LIMIT,
  })
  if (quotaError) {
