...
-- description: Add a tenant-scoped atomic RPC for creating UoM-aware purchase orders.
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(SUM(l.quantity * l.unit_price * (l.discount_percentage / 100)), 0),
    COALESCE(SUM(
      (l.quantity * l.unit_price * (1 - l.discount_percentage / 100))
      * (l.tax_percentage / 100)
    ), 0),
    COALESCE(SUM(l.line_total), 0)
  INTO v_gross, v_discount_amount, v_tax_amount, v_total
...
COMMENT ON FUNCTION public.rpc_create_uom_purchase_order(jsonb) IS
  'Atomically validates and creates one tenant-scoped UoM-aware purchase-order header and all lines. Header totals are derived from persisted normalized lines; any validation or insert failure rolls back the complete document.';
