...
> مولّد ومراجعته جارية في Draft PR #224؛ لا يُعد منشورًا على `main` قبل قرار دمج
> منفصل. التفاصيل في `docs/db/HR_MULTI_ORG_RLS_188_RUNBOOK.md`.
...
> **الموحَّد (100 → 121) — مرتَّب تصاعدياً**
...
| **108** | **دوال انحرافات التصنيع** (`calculate_material_variances`/`calculate_labor_variances`): نُقلت من `src/database/migrations/001_create_variance_functions.sql`؛ النسخة الأصلية اعتمدت جداول غائبة (`stock_moves`/`labor_entries`)؛ **حُصِّن التنفيذ في 117** (`to_regclass()` ⇒ فارغ بأمان)؛ search_path مُثبَّت في migration 111 | ✅ مطبَّقة (يوليو 2026) |
...
| **116** | **دمج سياسات user_roles (بداية دفعة RLS)**: 3 سياسات متداخلة (org_admin ALL + super_admin ALL + own SELECT) ⇒ 4 سياسات، واحدة لكل عملية، OR-merge حرفي بنفس الدلالة + `(SELECT auth.uid())`/`(SELECT is_super_admin())` لتقييم واحد لكل استعلام (InitPlan) + TO authenticated. **هذا نموذج الدمج المعتمد لبقية تحذيرات Multiple Permissive Policies (461)** | ✅ مطبَّقة + مُتحقَّق منها (يوليو 2026) |
| **117** | **دوال الانحرافات ترجع فارغاً بدل 42P01 (تقرير الانحرافات «لا ينبض»)**: `calculate_material_variances`/`calculate_labor_variances` كانتا تموتان دائماً لأن `stock_moves`/`labor_entries` غائبتان؛ الآن `to_regclass()` يفحص الوجود ⇒ فارغ بأمان، والاستعلام انتقل لـ EXECUTE ديناميكي. عند تفعيل جداول التتبع تعمل تلقائياً | ✅ مطبَّقة + مُتحقَّق منها (يوليو 2026) |
| **118** | **استبدال سياسات app.current_org_id الميتة (جذر «الداشبوردات لا تنبض»)**: 38 جدولاً (منها gl_entries/sales_invoices/customers/warehouses) سياستها الوحيدة ميتة ⇒ القراءة المباشرة صفر صف منذ 83؛ حُذفت 62 سياسة ميتة + سياسة SELECT عاملة (wardah_org_id) لكل جدول، والكتابة تبقى عبر RPCs حصراً؛ + حذف tenant_* على manufacturing_orders (get_current_tenant_id تسقط للمؤسسة الافتراضية — ثغرة) + مكررات user_organizations. تحقق حي: عضو يقرأ قيود مؤسسته | ✅ مطبَّقة + مُختبرة حيّاً (يوليو 2026) |
| **119** | **الموحِّد الآلي للسياسات المتداخلة (إتمام دفعة RLS)**: ~30 جدولاً متداخلاً (أزواج manage+select في HR والتنظيمية + مجموعات admin/own/super_admin) ⇒ سياسة واحدة لكل عملية بـ OR-merge حرفي (نموذج 116 آلياً)، تسمية `_sel/_ins/_upd/_del_m`، TO authenticated؛ + تشديد جدول backup على super_admin فقط. تحقق حي: صفر تداخل + smoke قراءة العضوية/المؤسسة/الأدوار | ✅ مطبَّقة + مُختبرة حيّاً (يوليو 2026) |
| **120** | **تحصين دوال SECURITY DEFINER الفئة C (إغلاق مراجعة كودكس — P0)**: 10 دوال قرائية ⇒ SECURITY INVOKER؛ 17 دالة كتابية/حسّاسة ⇒ حارسا `wardah_assert_org_member`/`wardah_assert_org_admin` + org مشتق من السجل المستهدف لا من المعامل؛ `upsert_stage_cost` ⇒ نمط الغلاف (نواة `_core` بلا EXECUTE للعملاء). إصلاحات وظيفية مرافقة: `approve_journal_entry`/`batch_post_journal_entries`/`reverse_journal_entry_enhanced` (org من صف القيد لا app.current_tenant_id الميت)؛ علة 42702 في `get_account_statement` (غموض أعمدة مع معاملات الإخراج — أُصلحت بتأهيل lwb.*)؛ `rpc_get_trial_balance` يقبل p_tenant=NULL ويرفض غير الأعضاء | ✅ مطبَّقة + مُختبرة حيّاً (يوليو 2026) — حارس مؤسستين يمر 28/28 |
| **121** | **fail-closed tenant resolver (إغلاق P0 عزل المؤسسات — مراجعة كودكس الحية 2026-07-16)**: كشفت المراجعة أن الفحص الختامي لـ120 كان false-negative — `wardah_org_id`/`get_current_tenant_id` نفساهما لم يتحققا من العضوية. `get_current_tenant_id` كانت تسقط للمؤسسة الافتراضية `000…001` في المسار الطبيعي وفي معالج EXCEPTION، و`wardah_org_id(p_explicit)` كانت تعيد قيمة العميل بلا فحص ⇒ هوية authenticated بلا عضوية كانت تقرأ وتكتب بيانات المؤسسة الافتراضية (مثبت حيّاً: 118 منتجاً + UPDATE ناجح). الإصلاح: `wardah_org_id` هو الجذر الأمني الموثّق — لا يعيد أبداً مؤسسة ليس المستخدم عضواً نشطاً فيها (NULL بدلاً من ذلك، بلا استثناء لأنه في مسند 99 سياسة RLS)؛ `get_current_tenant_id` بلا مؤسسة افتراضية إطلاقاً (claim الـJWT يُقبل بعد تأكيد العضوية، وإلا عضوية نشطة واحدة بترتيب حتمي (الأقدم) — يشمل متعدد المؤسسات فلا يفقدون الوصول، وإلا NULL)؛ دالة boolean جديدة `wardah_is_org_member`؛ حارس صريح على `rpc_create_mo_with_reservation` (الوحيدة بلا IF v_org IS NULL) مع **تصحيح أعمدة INSERT لتطابق المخطط الحي** (order_number/quantity/item_id بدل mo_number/qty_planned/uom_id الغائبة — إغلاق ملاحظة كودكس على الـPR)؛ فحص ختامي صارم يتحقق أولاً أن الجذرين محصَّنان (لا UUID افتراضي + فحص user_organizations) ثم يثق بهما. اختبار سلبي حي: `scripts/security/test_definer_org_guards.sql` | ✅ مطبَّقة + مُختبرة حيّاً (2026-07-16) — 11/11 اختبار: بلا عضوية ⇒ 0 صف قراءةً/كتابةً وRPC مرفوض؛ العضو الشرعي ⇒ 118 منتجاً + إنشاء أمر تصنيع ينجح؛ p_explicit=OrgB وclaim مزوّر ⇒ مرفوضان؛ متعدد المؤسسات ⇒ يحتفظ بالوصول لإحدى مؤسساته |
...
- **حدود التمثيل**: المستودع يمثّل 101–121 حرفياً/دلالياً (تحقق 16 يوليو 2026)،
  لكنه **لا يوفر بعد baseline مستقلاً** لإعادة بناء المخطط التاريخي (66–99) من الصفر.
  يُغلق نهائياً بـ T2 (schema baseline من الإنتاج — راجع CODEX_REVIEW_REMEDIATION_TODO.md).
...
### تحقق التطابق الحرفي GitHub ↔ الإنتاج (16 يوليو 2026)

قورن نص كل migration في `supabase_migrations.schema_migrations` مع ملف
المستودع المقابل بـ **normalized-md5** (بعد إزالة التعليقات والمسافات):

- **118 و119 كانتا ملفي توثيق بلا SQL تنفيذي** — استُبدلتا بالنص المطبَّق
  الحقيقي من سجل Supabase. الملفان الآن يعيدان إنتاج الإصلاح كاملاً.
- **103/104/110/111/112 كانت متباعدة عن المطبَّق** وأُصلحت: `RAISE NOTICE`
  عارية خارج `DO` في 103/110/111 (SQL غير صالح — كان يُفشل أي إعادة بناء)،
  ونسخة قديمة من `rpc_get_invitation_preview` ترجع JSONB بدل TABLE في
  104/112، وJOIN غير مطبَّق في 111.
- **النتيجة**: 17/19 تطابق حرفي (md5)، و102/107 تطابق دلالي كامل
  (تحقق جملةً جملة — الفرق نص تعليق/اسم متغير فحسب).
- **حارس الاستمرارية**: خطوة CI جديدة `Migration SQL syntax check`
  (`scripts/ci/check_migration_syntax.py` بمحلل libpg_query) تفحص 156 ملفاً
  وترفض أي صياغة لا يقبلها PostgreSQL قبل الدمج.

### اختبار بناء قاعدة فارغة (16 يوليو 2026)

نُفِّذت السلسلة القانونية كاملة (148 ملفاً بترتيب هذا الدليل) على
PostgreSQL 16 نظيف مع محاكاة Supabase (`scripts/ci/fresh-db/`):
**60 نجحت / 87 فشلت / 1 لم تُشغَّل (119 — توقفت السلسلة بعد فشل 118) = 148** — الجداول الأساسية أُنشئت تاريخياً خارج السلسلة
(SQL Editor / `sql/archive/`) فلا يعيد بناءها أي ملف مرقّم.
التفاصيل والعلاج (schema baseline من الإنتاج ثم تفعيل job الـCI):
`docs/db/FRESH_DB_REBUILD_REPORT.md`. **الخلاصة العملية: بيئة جديدة
تُبنى حالياً من baseline يُسحب من الإنتاج، لا من إعادة تشغيل السلسلة.**

## فجوات الترقيم الموثَّقة (أرقام لا ملفات مقابلة لها)

هذه الأرقام لا توجد كملفات في `sql/migrations/` لأنها استُخدمت في مرحلة التطوير
المبكرة عبر SQL Editor مباشرةً (قبل اعتماد مسار الملفات المرقّمة) ثم حُذفت أو
أُعيد ترقيمها. موثَّقة هنا لإسكات فحص CI (`run_chain.sh`).

| الأرقام المتجاوزة | السبب |
|---|---|
| 34, 35, 36, 37 | سكربتات تجريبية مؤقتة — حُذفت قبل اعتماد مسار الملفات (يناير–مارس 2025) |
| 39 | محجوز لمحاولة migration فشلت ولم تُطبَّق — استُبدلت بـ 40 |
| 42, 43, 44, 45, 46, 47, 48, 49 | نطاق تطوير WMS الأولي — أُلغيت وأُعيد هيكلة نظام المستودعات في 50+ |

> الأرقام الموثَّقة أعلاه مُستثناة من فحص التسلسل — لا ملفات مقابلة ولا يجب إنشاؤها.
...
3. **أرقام جديدة**: التالي هو **121**. أي migration جديدة = ملف جديد مرقّم + سطر هنا.
   ...
   — 120: تحصين دوال SECURITY DEFINER الفئة C (28 دالة) — حارسا wardah_assert_org_member/wardah_assert_org_admin، 10 دوال ⇒ INVOKER، غلاف upsert_stage_cost.
