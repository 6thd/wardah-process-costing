...
| **93** | **إتمام أمر تصنيع ذرّي: مخزون تام (متوسط مرجّح) + سلسلة قيود Raw→WIP→FG (Fail-closed) + idempotent** | ✅ مطبَّقة + مُختبرة حيّاً (PART 1: مخزون تام + done + replay) |
| **94** | **إغلاق ذرّية المخزون: SLE + bins + تقييم (FIFO/LIFO/متوسط) داخل rpc_post_goods_receipt** | ⏳ مكتوبة — بانتظار التطبيق الحيّ + اختبار rollback (كود الواجهة/الاختبارات خضراء) |
...
4. **✅ حُسم تعارض مسار التسليم (Migration 87) ثم تحصينه (88)**: القانوني هو
   ...
   **89** يوفّر استلاماً ذرّياً مماثلاً (رأس+سطور+GRNI fail-closed). كلاهما مُختبَر
   ...
   بانتظار التطبيق الحيّ + اختبار rollback** (كود/اختبارات الواجهة خضراء).
