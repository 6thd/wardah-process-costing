...
5. **Mapping-drift A/B/C (twenty-sixth / P2-1; observed gates: twenty-seventh).** All three are **test-instrumented** candidates, not production 191. Shared rule: observed `wait_event_type = 'Lock'` and `pg_blocking_pids(reservation_pid) = {gate_holder_pid}` **before** mapper COMMIT; prove that commit finished before releasing the gate; sleep-only placement is rejected. Runtime gates and the static source-shape checks are complementary. **A** (`K`, after prefix, before first bins query): `Y` is a valid same-org product with **no** other active/current `item_product_map` row at mapper time. In-place UPDATE (`product_id` X→Y) + COMMIT must succeed while gated; a uniqueness violation is a harness/fixture failure. After release, capture-bound must block on `blocker_X`; blocking on `blocker_Y` is a FAIL; `Y`'s bin must never be acquired. **B** (`K_B`, after availability/MO creation, before first reservation INSERT): same `Y` uniqueness precondition and in-place UPDATE (preserves already-eligible `valid_from`, so C's backdating does not apply); uniqueness violation is a harness/fixture failure. INSERT supplies captured `X`; trigger/`RETURNING` is `Y`; `Y != captured X` raises `ITEM_PRODUCT_MAPPING_DRIFT`; whole-RPC rollback (MO must not persist; no reservation persists). **FAIL** if INSERT commits with `Y`, mismatch is ignored, or MO survives. **C** (`K_C`, same persistence-time boundary as B): legal in-prefix swap `I1↔I2` respecting uniqueness (retire old current mappings, then INSERT new ones — not two naive `product_id` UPDATEs). Capture `v_candidate_mapping_at` from the candidate transaction (= trigger `p_at`). New mappings must have explicit `valid_from <= v_candidate_mapping_at` (copy old eligible `valid_from` or a deterministic earlier timestamp); **do not** use `DEFAULT now()`. After mapper COMMIT and **before** releasing `K_C`: `wardah_resolve_product_id(org, I1, v_candidate_mapping_at) = Y` and `(org, I2, …) = X` — old identity or `ITEM_PRODUCT_MAP_MISSING` here is a harness/fixture failure. Then release; captured vs `RETURNING` is `X/Y` vs `Y/X`; `ANY(prelocked_set)` would accept; exact identity raises `ITEM_PRODUCT_MAPPING_DRIFT`; whole-RPC rollback. `ITEM_PRODUCT_MAP_MISSING` after release does not satisfy C.
...
- Fix G mapping-drift A observed-gate choreography from §9a (not a sleep-placed remap; blocking on `blocker_Y` is a FAIL; in-place X→Y requires `Y` unmapped at mapper time; uniqueness violation is a harness/fixture failure; production 191 has no gate);
- Fix G mapping-drift B observed-gate choreography from §9a (`K_B` after availability/MO, before first reservation INSERT; in-place X→Y preserves `valid_from`; `Y` unmapped at mapper time; uniqueness violation is a harness/fixture failure; whole-RPC rollback including the MO; production 191 has no gate);
- Fix G mapping-drift C observed-gate choreography from §9a (`K_C` at B's boundary; legal in-prefix swap with explicit `valid_from <= v_candidate_mapping_at`; pre-release resolver assertions; `ITEM_PRODUCT_MAP_MISSING` is a fixture failure; `ANY(prelocked_set)` would false-pass; production 191 has no gate);
...
- How are deterministic races proven? — main design §7, including Fix G Control A/B, the real-M191 lock-mode probe, and mapping-drift A/B/C observed gates.
