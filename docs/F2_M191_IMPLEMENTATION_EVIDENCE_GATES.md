...
5. **Mapping-drift A/B/C (twenty-sixth / P2-1; observed gates: twenty-seventh).** All three are **test-instrumented** candidates, not production 191. Shared rule: observed `wait_event_type = 'Lock'` and `pg_blocking_pids(reservation_pid) = {gate_holder_pid}` **before** mapper COMMIT; prove that commit finished before releasing the gate; sleep-only placement is rejected. Runtime gates and the static source-shape checks are complementary. **A** (`K`, after prefix, before first bins query): mapper COMMITs `I: X→Y`; after release, capture-bound must block on `blocker_X`; blocking on `blocker_Y` is a FAIL; `Y`'s bin must never be acquired. **B** (`K_B`, after availability/MO creation, before first reservation INSERT): mapper COMMITs `I: X→Y`; INSERT supplies captured `X`; trigger/`RETURNING` is `Y`; `Y != captured X` raises `ITEM_PRODUCT_MAPPING_DRIFT`; whole-RPC rollback (MO must not persist; no reservation persists). **FAIL** if INSERT commits with `Y`, mismatch is ignored, or MO survives. **C** (`K_C`, same persistence-time boundary as B): legal in-prefix swap `I1↔I2` respecting `uq_item_product_map_current_item` / `uq_item_product_map_current_product` (retire old current mappings, then install new ones — not two naive `product_id` UPDATEs); trigger results are in `{X,Y}` so `ANY(prelocked_set)` would accept; exact per-line identity must raise `ITEM_PRODUCT_MAPPING_DRIFT` on the first mismatching INSERT; whole-RPC rollback.
...
- Fix G mapping-drift A observed-gate choreography from §9a (not a sleep-placed remap; blocking on `blocker_Y` is a FAIL; production 191 has no gate);
- Fix G mapping-drift B observed-gate choreography from §9a (`K_B` after availability/MO, before first reservation INSERT; whole-RPC rollback including the MO; production 191 has no gate);
- Fix G mapping-drift C observed-gate choreography from §9a (`K_C` at B's boundary; legal in-prefix swap; `ANY(prelocked_set)` would false-pass; production 191 has no gate);
