...
>
> **تحديث الحالة (2026-07-17)**: اكتملت T6 بالكامل (PR #23 مدموج في main):
...
**المرجع**: `docs/db/FRESH_DB_REBUILD_REPORT.md` + `sql/baseline/README.md`.

**ما أُنجز (2026-07-17)**:
- `sql/baseline/` مُنشأ مع `README.md` يشرح عملية التوليد.
- `.github/workflows/generate-baseline.yml` مُنشأ: workflow يدوي يشغّل
  `pg_dump "$SUPABASE_DB_URL" --schema-only --no-owner --no-acl --schema=public`
  ويُودع الناتج تلقائياً مع `-- migration_cutoff: N` في أول سطر.

**ما تبقى — خطوة واحدة يدوية**:
1. أضف سر `SUPABASE_DB_URL` في `Settings → Secrets → Actions`:
   ```
   postgresql://postgres:[DB_PASSWORD]@db.uutfztmqvajmsxnrqeiv.supabase.co:5432/postgres?sslmode=require
   ```
2. شغّل `Actions → Generate Schema Baseline → Run workflow`.
3. سيُودع `sql/baseline/000_schema_baseline_YYYYMMDD.sql` تلقائياً ⇒
   T3 ينشّط تلقائياً في CI.

**معيار القبول** (بعد التوليد): `psql -f supabase_shim.sql && psql -f baseline`
على PostgreSQL 16 نظيف ينجح بلا أخطاء.

### ◐ T3 — fresh-db CI ينجح 100% (وليس توثيق 87 فشلًا)

**ما أُنجز (2026-07-17)**:
- `services: postgres:16` أُضيف لـjob `test` في `ci-cd.yml` (إصلاح «لا خادم PostgreSQL»).
- خطوة `Fresh DB chain test` أُعيد كتابتها بالكامل:
  - تستخرج `migration_cutoff` من `-- migration_cutoff: N` في أول سطر الـbaseline.
  - تمرّر `CUTOFF` لـ`build_apply_order.py` ⇒ يُخرج فقط migrations بأرقام > CUTOFF.
  - حذف `BASELINE_TS` (المتغير الغير مستخدم) + إضافة `PGPASSWORD`.
  - إن الـbaseline غائب: `exit 0` بدون فشل (T2 معلق بشكل واضح).
  - إن الـbaseline موجود والـmigrations صفر: `exit 0` (لا جديد بعد cutoff).
  - إن الـmigrations > 0: `run_chain.sh` + فحص `FAIL=0 AND NOT_RUN=0`.
- `build_apply_order.py` يقبل الآن معامل `cutoff` اختيارياً (sys.argv[2], default=-1).
  يتجاهل الـunnumbered files إن `cutoff >= 30`.

**ما تبقى**: تفعيل T3 فعلياً عند إنجاز T2 (الخطوة اليدوية أعلاه).

**معيار القبول**: الـjob أخضر على PR فارغ بعد T2، ويفشل عند دسّ migration
بصياغة خاطئة (اختبار سلبي واحد موثق).

**فجوات مؤكَّدة — مُغلقة ضمن هذه الدفعة**:
- ~~لا خدمة PostgreSQL معرَّفة في `ci-cd.yml`~~ ✅ مُصلح.
- ~~`BASELINE_TS` يُحسب ولا يُستخدَم~~ ✅ مُحذوف.
- ~~`build_apply_order.py` لا يقبل cutoff~~ ✅ مُضاف.
...
**ملاحظة نطاق البوابة**: تكشف نمط `isRTL ? '<عربي>'` فقط. النصوص العربية
المشفَّرة خارج هذا النمط (مثل labels وأوصاف بدون مقابل إنجليزي مباشر) تبقى
خارج النطاق الحالي — تُعالَج ضمن مهمة مستقلة مستقبلاً.
...
| FU-5 | إصلاح **SonarQube Quality Gate** الفاشل على `main` وعلى PRs (شرط «0% تغطية على الكود الجديد ≥80%») | P1 | CI/CD Pipeline (Test & Build) أخضر؛ Sonar Gate فقط فاشل — يفشل على كل PR يضيف ملفات SQL/سكربت غير قابلة للتغطية |
