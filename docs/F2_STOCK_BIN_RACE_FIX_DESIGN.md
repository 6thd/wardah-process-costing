...
| New: two different multi-line callers (e.g. one `rpc_post_goods_receipt`, one `rpc_post_delivery_note`), overlapping products A and B in reversed line order (call 1: A then B; call 2: B then A), no pre-existing bins for one side | **Fix E omitted, deterministic RED (eighteenth correction):** "start both concurrently and hope" is not acceptable on its own — nothing forces either call to hold its first product at the exact instant the other needs it. Force it with an external, test-only blocker and PostgreSQL's FIFO ordering of conflicting lock requests: an external session locks product B first and holds it; start call 2 (needs B first) — it queues behind the blocker, confirmed via `pg_blocking_pids`; start call 1 (needs A first) — it locks A uncontested, then reaches for B and also queues behind the blocker, *after* call 2 (confirmed queue order); release the blocker — FIFO grants B to call 2 (queued first), not call 1; call 2 now holds B and reaches for A, held by call 1 — the cycle is complete (call 1 waits on call 2 for B; call 2 waits on call 1 for A) and must produce a genuine `deadlock detected` (`40P01`). No instrumentation of either real multi-line caller is needed — the blocker and release ordering alone force the shape. **Fix E present, real 191:** identical fixture (same blocker, same start order) must complete both calls without deadlock, regardless of which call's `wardah_lock_products_for_stock_write` commits first |
...
| New: incoming vs `rpc_create_mo_with_reservation`, same product with an existing bin | **Deterministic choreography (eighteenth correction), no instrumentation of either real function:** an external, test-only session locks the target bin row and holds it. Start the real, unmodified `rpc_create_mo_with_reservation` — it queues behind the external blocker at its own bin lock, confirmed via `pg_blocking_pids`. Start `wardah_apply_stock_incoming` with the products lock as `FOR UPDATE` (the pre-fix mutant) — it locks the product uncontested, then also queues behind the external blocker for the bin, *after* reservation (confirmed queue order — two requesters, same conflicting mode, on one row). Release the external blocker: PostgreSQL's documented FIFO ordering for conflicting lock requests on one row grants the bin to reservation (queued first), not incoming; reservation then attempts its FK-implied `FOR KEY SHARE` on the product, held by incoming's `FOR UPDATE` — conflict; incoming is still waiting on the bin, now held by reservation — the cycle is complete and must produce a genuine `deadlock detected` (`40P01`), confirmed empirically against a live PostgreSQL 17 instance. **Run against real 191** (`FOR NO KEY UPDATE`): identical choreography, same external blocker and release order — reservation's `FOR KEY SHARE` no longer conflicts with incoming's held lock, so no cycle forms; both complete, bin and reservation both correct regardless of which serializes first |
...
the ordering guarantee for Fix C comes entirely from (1) above plus the
...
- the "Fix E omitted" multi-line-caller RED fixture or the "incoming vs
  ...
  blocker on the second-needed shared resource plus PostgreSQL's documented
  FIFO ordering of conflicting lock requests on one row to force a specific,
  confirmed queue order before releasing it — no instrumentation of either
  real function is needed, but a bare concurrent start is not sufficient
  (the eighteenth correction above)
