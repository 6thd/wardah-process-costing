...
**الحالة:** 🟢 **مطبَّقة على Production ومتحقَّق منها**
**الدمج:** PR #194 · `main@cdbe648cb2c64d9c4cbcf39daf4dcc7b24376e01`
**التطبيق:** `20260828214043` · `182_trial_balance_ledger_truth` · صف واحد في سجل Production
...
## 6. تطبيق Production والتحقق (مكتمل)
...
تُعيد 182 لاحقًا تعريفًا أقدم قائمًا على العضوية فقط.

تحذير Supabase Advisor من كون الدالة `SECURITY DEFINER` قابلة للاستدعاء من
`authenticated` ليس ضجيجًا عامًا: هو نفس فجوة `SEC-172` المسجلة في
`scripts/ci/security/rbac-mutation-classification-reviewed.json` — القراءة محروسة
حاليًا بعضوية المؤسسة، لكنها لا تفرض بعد صلاحية القراءة المحاسبية/التقارير الدقيقة.
Migration 183 هي التي يجب أن تغلق هذا التحذير مع الحفاظ كاملًا على جسم 182.
...
نُفِّذ على Production في 2026-08-28، وسُجّل مرة واحدة بالنسخة
`20260828214043` كرأس سجل الترحيل.
...
`rpc_get_trial_balance(<org>, CURRENT_DATE)` وتحقق من تساوي مجموع
`closing_debit` و`closing_credit`، ثم قارنهما بحساب مستقل **لصافي رصيد كل حساب**
من الدفتر القانوني. لا تساوِ صافي أرصدة الإقفال بإجمالي دوران الأسطر.

إذا كان إجمالي حركة الحساب `i` هو `D_i` مدينًا و`C_i` دائنًا، فالدالة تعرض
`GREATEST(D_i - C_i, 0)` على الجانب المدين أو العكس على الجانب الدائن. لذلك، في
دفتر متوازن:

```text
دوران كل جانب - صافي أرصدة كل جانب = SUM(LEAST(D_i, C_i)) لكل حساب
```

### 6.3.1 النتيجة الفعلية على Production

نُفِّذ التحقق تحت دور `authenticated` مع `auth.uid()` لمستخدم حقيقي وعضوية نشطة،
وليس تحت `service_role`:

| القياس | النتيجة |
|---|---:|
| صفوف الحسابات | 12 |
| الافتتاحي المدين / الدائن | 16,790.00 / 16,790.00 |
| حركة الفترة المدينة / الدائنة | 10,895.00 / 10,895.00 |
| صافي الإقفال المدين / الدائن | 27,685.00 / 27,685.00 |
| دوران الدفتر المدين / الدائن | 30,805.00 / 30,805.00 |
| الفرق في كل جانب | 3,120.00 |
| `SUM(LEAST(D_i, C_i))` | 3,120.00 — مطابق للفرق |
| المقارنة حسابًا بحساب | 12/12 مطابقة · صفر اختلافات |

الحساب `1130` يفسّر المقاصّة كلها: `10,400.00` مدين و`3,120.00` دائن، فيظهر
بصافي مدين `7,280.00`. فالفرق خاصية رياضية لميزان المراجعة، لا نقص بيانات.

ظهرت الرموز السبعة في §3.1 كلها، وتحقق شكل metadata المتوقع: الاسم = الرمز،
الاسم العربي `NULL`، والنوع سلسلة فارغة.
...
- [x] تطبيق 182 على Production مرة واحدة (`20260828214043`)
- [x] تحقق بنيوي: المصدر القانوني حاضر، التاريخي غائب، overload واحد والمنح محفوظة
- [x] تحقق وظيفي بهوية `authenticated`: 12 حسابًا، متوازن، وصفر اختلافات

### ⏳ ما تبقّى — خارج نطاق هذا الـPR

- [ ] **SEC-172** — إضافة صلاحيات القراءة المالية الدقيقة في Migration 183 فوق
      تعريف 182؛ لا يجوز أن تعيد الدالة إلى مصدر أو حارس أقدم. هذا هو بند
      Supabase Advisor الخاص بإتاحة دالة `SECURITY DEFINER` لـ`authenticated`.
...
1. طُبّقت الـmigration على Production وتُحقّق منها في 2026-08-28. لم تُطبَّق على
   Staging ضمن هذه الجولة، ولم تغيّر أي بيانات محاسبية؛ استبدلت تعريف الدالة وأضافت
   صف سجل الترحيل فقط.
2. القبول المحلي جرى على **PostgreSQL 16**، وCI على **17**، ثم نُفّذ التطبيق
   والتحقق الفعليان على Supabase PostgreSQL **17.4** دون فارق يمس منطق الدالة.
