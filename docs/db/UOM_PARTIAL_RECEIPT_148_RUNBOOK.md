...
## 10. نتيجة Pilot الإنتاجي — 2026-08-22

نجح الاستلام الجزئي من `UomGoodsReceiptForm` والتحويل إلى وحدة الأساس وحركة
المخزون وGRNI وحالة PO وإعادة الطلب idempotent. كشف أول طلب جديد مستقل بعده
تصادمًا في مولّد `receipt_number` قبل بلوغ بوابة `OVER_RECEIPT`: الأرقام
التاريخية الطويلة كانت تُقتطع عبر `lpad(..., 6)` إلى نفس البادئة القصيرة.

المعاملة الفاشلة تراجعت بالكامل ولم تترك رأسًا أو سطرًا أو حركة أو قيدًا. الإصلاح
الإضافي ومسار الدمج والتطبيق والتحقق موثقان في:

`docs/db/GOODS_RECEIPT_NUMBER_SEQUENCE_177_RUNBOOK.md`.

لا يُغلق Issue #45 قبل دمج Migration 177 وتطبيقها على Production ثم إعادة
الاختبار السلبي وإثبات عدم التكرار.
