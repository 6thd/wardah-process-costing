...
- بعد تطبيق 184، أي `UPDATE` يلمس أحد الرؤوس التاريخية الثلاثة وهي ما تزال بلا
  سطور قانونية سيُرفض بـ`POSTED_ENTRY_LINES_MISSING`. لذلك أي معالجة لاحقة لها
  يجب أن تُدخل السطور الموثوقة وتُجري أي تعديل لازم على الرأس داخل المعاملة
  الذرية نفسها، لا أن تعدّل الرأس منفردًا أولًا.
...
- **RLS/deferred regression:** مستخدم `authenticated` عضو في مؤسستين وبدون
  ...
  `SECURITY INVOKER` أو صار fail-open عبر RLS. خطوة anti-vacuity في الـworkflow
  تشترط أيضًا ظهور `GL_184_RLS_FAIL_CLOSED_OK` صراحةً حتى لا يمكن حذف هذا
  الإثبات مستقبلًا مع بقاء البوابة خضراء.
