...
pairE gr-vs-dn \
"SELECT public.rpc_post_goods_receipt(jsonb_build_object('tenant_id','$org','vendor_id','$VEND','warehouse_id','$W','idempotency_key','s12-gr-1','lines',$GR_LINES_AB));" \
"SELECT public.rpc_post_delivery_note(jsonb_build_object('tenant_id','$org','sales_invoice_id','$INV','warehouse_id','$W','idempotency_key','s12-dn-1','lines',$DN_LINES_BA));"
