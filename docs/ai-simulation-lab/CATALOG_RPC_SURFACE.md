...
**وسطح العميل الكامل أوسع:** 182 دالة ممنوحة لـ`authenticated`، منها 118 خارج
تسمية `rpc_` و24 منها قادرة على الكتابة — انظر §17 قبل تصميم أي سيناريو.
...
معطّلين دون أن تلاحظ. التحقق من الـ258 صفًا شرط قبول في Phase 0.
...
## 17. ⚠️ سطح العميل أوسع من الـRPCs — 118 دالة غير مسمّاة `rpc_`

**اكتشاف من اشتقاق المنح، يغيّر تقدير سطح الخطر:**

| القياس | العدد |
|---|---|
| إجمالي دوال `public` الممنوحة لـ`authenticated` | **182** |
| منها `rpc_*` (العقود القانونية) | 64 |
| منها **غير** `rpc_*` (سطح تاريخي) | **118** |
| من الـ118: قادرة على الكتابة (INSERT/UPDATE/DELETE في جسمها) | **24** |
