...
Round 3 **بدأت ولم تُغلق**. أُنجزت ثلاثة إغلاقات محددة، بينما بقيت عيوب سلامة
...
| جرد `stock_movements`/`stock_moves`/`avg_rate` | 🟡 مكتمل فنيًا، غير مدموج | PR #211 عند `a74c06f`؛ 11/11 checks، والخيطان محلولان، لكنه ما زال مفتوحًا |
...
1. دمج PR #211 بتفويض المالك، ثم تحديث `main` دون drift، ودمج مزامنة وثائق
   المختبر المتسلسلة عليه بتفويض مستقل.
2. PR-1R يحسم دوال `stock_moves` الحية: تقاعد موثق أو إعادة توجيه إلى
   `stock_ledger_entries`/`bins` عبر migration إضافية، مع Red/Green وFresh DB.
3. `rpc_create_mo_with_reservation` يعمل بمواد غير فارغة عبر العقد القانوني، أو
   يُزال من سيناريو اليوم بقرار منتج موثق؛ لا fallback تاريخي.
4. إغلاق `INV-02` باختبار idempotency/سباق حتمي قبل فرض قيد على Production.
5. إغلاق `INV-01` و`INV-03` بعقد bin/continuity قابل للفرض، مع فصل إصلاح البيانات
   التاريخية عن تغيير المخطط.
6. حسم `INV-04` كـprojection قانوني أو مرجع غير ملزم، ثم تعديل الثابت وفق القرار.
7. تحديث الكتالوجات من أحدث Baseline بعد كل migration، وإعادة إثبات D-1.
8. عدم وجود S0/S1 مفتوح يؤثر على سيناريو اليوم؛ الاستثناء المقبول—إن وُجد—يحتاج
...
|---:|---|---|---|
| 1 | مزامنة وثائق AI Simulation Lab | docs-only | هذا التحديث متسلسل على #211؛ يُراجع الآن ولا يغيّر قاعدة البيانات |
| 2 | قرار دمج PR #211 | قرار مالك | لا ينفذ دون تفويض صريح منفصل |
| 3 | تثبيت مزامنة الوثائق على `main` | docs-only | بعد #211 وفحص عدم drift؛ تحتاج قرار دمج مستقلًا |
| 4 | PR-1R لعقود `stock_moves` | DB-first | preflight → migration إضافية → Red/Green → Fresh DB؛ لا Production apply ضمن PR |
| 5 | idempotency للمخزون (`INV-02`) | DB | duplicate preflight + سباق حتمي + قيد قانوني |
| 6 | bin/continuity (`INV-01`/`INV-03`) | DB/data split | عقد مستقبلي منفصل عن remediation التاريخي |
| 7 | projection والهوية (`INV-04`/`INV-06`) | DB/application حسب القرار | مصدر حقيقة واحد واختبار مستهلك |
| 8 | UoM/org/provenance (`INV-07`/`INV-08`/`INV-10`) | follow-ups محددة | بلا backfill استنتاجي |
| 9 | Phase 0 | مختبر معزول | فقط بعد بوابة Round 3؛ Issue #195 يبقى مسارًا مستقلًا |
