...
> **تحديث لاحق — 2026-09-26:** ما أدناه وصف تاريخي لحالة 2026-09-02.
> نُفّذ على Production تصحيح بيانات Pilot التجريبية بتفويض المالك، بعد
> مراجعة مستقلة للنص عند `ab6edba60f5775f1284d33b15f2ac8a9785d17c8`.
> حُفظت حركتا `ADJ-000001` المكررتان وحركة `RM-042` وحركتا `RM-010`، لكن
> أصبحت الخمس `is_cancelled=true, docstatus=2` بلا صفوف عكسية؛ هذه تسوية
> تاريخية مقصودة، فتُظهر التقارير المؤرخة رصيدًا صفريًا لها بأثر رجعي.
> بحسب قراءة المنفذ بعد الالتزام، لم تبق حركات نشطة ولا فروقات في
> `validate_stock_balance` ضمن المؤسسة `00000000-…-0001`، وأرصدة الأصناف
> الأربعة `001`/`PP810837`/`RM-010`/`RM-042` صفر في المنتج والمستودع.
> أُلغيت الاستلامات الثلاثة وأمرا الشراء وقيدا GL المسودتان والتسوية.
> وُثّقت اللقطة في `audit_logs` بإجراء `experimental_fixture_cleanup`
> ومعرّف الحدث `2026-09-26-M191-preflight` عند `2026-09-26 13:07:51 UTC`؛
> لا يتضمن هذا الـPR سجل قراءة خامًا مستقلًا. حالة `INV-01` التاريخية
> وتكرار `ADJ-000001` لم يعودا نشطين؛ عقد منع تكرار الحركات
> واستمرارية الرصيد مستقبلًا مسار مستقل. بحسب قراءة المنفذ، سجل Production
> ما زال عند M189؛ لم يُطبّق M190 أو M191.
