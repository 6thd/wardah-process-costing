...
> اليوم كاملًا. جرد PR #211 أثبت أن `rpc_create_mo_with_reservation` تقرأ
> `stock_moves` الغائبة عند مواد غير فارغة؛ لذلك `SC-DAY-01` و`SC-RACE-08`
> محجوبان حتى قرار `OQ-09` وتنفيذ PR-1R. لا يُستخدم fallback تاريخي لتجاوز الحاجز.
...
**الحالة الحالية:** `BLOCKED` قبل Phase 1. خطوة إنشاء أوامر التصنيع بمواد غير
فارغة تصل إلى relation `stock_moves` غير الموجودة. الاجتياز المطلوب ليس قبول
خطأ 42P01 بوصفه رفضًا متوقعًا؛ المطلوب إصلاح العقد القانوني أو تغييره بقرار منتج
مراجع (`OQ-09`).
...
| إنشاء MO بمواد عبر العقد القانوني | `rpc_create_mo_with_reservation` نفسها تقرأ `stock_moves` الغائبة | `OQ-09` / PR-1R |
