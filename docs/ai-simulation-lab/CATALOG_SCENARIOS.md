...
> اليوم كاملًا. جرد PR #211 أثبت عطل `rpc_create_mo_with_reservation`، وأصلحته
> Migration 186 المدموجة والمطبقة مع قفل واختبار سباق. يبقى `SC-DAY-01` و
> `SC-RACE-08` محجوبين حتى تجهيز بيئة المختبر وإغلاق بقية بوابة Round 3؛
...
| 13 | Production Planner | محدود | `rpc_create_mo_with_reservation` | العقد أصلحته Migration 186؛ التشغيل ينتظر بيئة المختبر |
...
**الحالة الحالية:** `BLOCKED` قبل Phase 1 بسبب عدم تجهيز بيئة المختبر وبقاء
بوابة Round 3، لا بسبب 42P01. Migration 186 أزالت العطل، نجحت في Fresh DB
والسباق، وطُبقت على Production؛ تشغيل السيناريو نفسه يبقى في البيئة المعزولة
بعد تجهيزها (`OQ-09`).
...
| تشغيل إنشاء MO بمواد داخل المختبر | العقد أُصلح في Migration 186؛ لم تُجهز بيئة المختبر ولم تُنفذ تشغيلة السيناريو بعد | `OQ-09` / PR #213 |
...
**الحالة الحالية:** `BLOCKED` حتى تجهيز بيئة المختبر. العقد القانوني لم يعد
يفشل على relation غائبة بعد Migration 186، فأصبحت المقارنة ذات معنى؛ لكن لا
تُنفذ على Production ولا يُعاد تعريف نجاحها إلى «الدالتان تفشلان» لأن ذلك أخضر
كاذب.
