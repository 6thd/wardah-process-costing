...
// ===== percentage_base مفعّل فعلياً (P2-13: كان المعامل يُستقبل ويُتجاهل) =====
describe('evaluateComponentAmount — أساس النسبة', () => {
  it('basic (الافتراضي): نسبة من الأساسي فقط', () => {
    expect(evaluateComponentAmount('percentage', 10, 8000)).toBe(800);
    expect(evaluateComponentAmount('percentage', 10, 8000, 'basic', 2000)).toBe(800);
  });
  it('basic_housing: نسبة من الأساسي + السكن', () => {
    expect(evaluateComponentAmount('percentage', 10, 8000, 'basic_housing', 2000)).toBe(1000);
  });
  it('أساس مجهول ⇒ رفض واضح لا fallback صامت', () => {
    expect(() => evaluateComponentAmount('percentage', 10, 8000, 'gross')).toThrow(/أساس نسبة غير معروف/);
  });
  it('fixed يتجاهل الأساس', () => {
    expect(evaluateComponentAmount('fixed', 1234, 0, 'basic_housing', 999)).toBe(1234);
  });
});

describe('calculatePayrollPreview — percentage_base تكاملياً', () => {
  it('بدل بنسبة من basic_housing يُحسب بعد اكتمال السكن، ومكوّن سكن دائري يُرفض', async () => {
    vi.mocked(getHrPolicies).mockResolvedValue({
      gosi_employee_pct: 0, gosi_employer_pct: 0, gosi_base_cap: 45000,
      gosi_applies_to: 'none', weekend_days: ['friday'], employee_daily_hours: 8,
      overtime_grace_minutes: 0, overtime_multiplier: 1.5,
      daily_rate_basis: 'working_days', overtime_base: 'basic',
    } as never);
    vi.mocked(getPayrollLock).mockResolvedValue(null as never);
    vi.mocked(listAdjustmentsForMonth).mockResolvedValue([] as never);
    vi.mocked(listAttendanceForPeriod).mockResolvedValue([] as never);
    vi.mocked(supabase.from).mockImplementation(((table: string) => {
      if (table === 'employees') {
        return thenable({
          data: [{ id: 'emp-1', employee_id: 'E-1', first_name: 'س', last_name: '',
                   department: null, salary: 0, status: 'active', is_saudi: false }],
          error: null,
        });
      }
      if (table === 'employee_salary_structures') {
        return thenable({
          data: [
            { employee_id: 'emp-1', value: 8000, component: { code: 'BASIC', component_type: 'earning', calculation_type: 'fixed' } },
            // سكن 25% من الأساسي = 2000
            { employee_id: 'emp-1', value: 25, component: { code: 'HOUSING', component_type: 'earning', calculation_type: 'percentage', percentage_base: 'basic' } },
            // بدل آخر 10% من (أساسي+سكن) = 1000
            { employee_id: 'emp-1', value: 10, component: { code: 'SPECIAL', component_type: 'earning', calculation_type: 'percentage', percentage_base: 'basic_housing' } },
          ],
          error: null,
        });
      }
    ...
    const preview = await calculatePayrollPreview(2025, 3);
    const emp = preview.employees[0];
    expect(emp.housingAllowance).toBe(2000);
    expect(emp.otherAllowance).toBe(1000); // 10% × (8000+2000)
    expect(emp.gross).toBe(11000);

    // مكوّن سكن بنسبة من basic_housing = اعتماد دائري ⇒ رفض واضح
    vi.mocked(supabase.from).mockImplementation(((table: string) => {
      if (table === 'employees') {
        return thenable({ data: [{ id: 'emp-1', employee_id: 'E-1', first_name: 'س', last_name: '', department: null, salary: 0, status: 'active', is_saudi: false }], error: null });
      }
      if (table === 'employee_salary_structures') {
        return thenable({
          data: [
            { employee_id: 'emp-1', value: 8000, component: { code: 'BASIC', component_type: 'earning', calculation_type: 'fixed' } },
            { employee_id: 'emp-1', value: 25, component: { code: 'HOUSING', component_type: 'earning', calculation_type: 'percentage', percentage_base: 'basic_housing' } },
          ],
          error: null,
        });
      }
    ...
    await expect(calculatePayrollPreview(2025, 3)).rejects.toThrow(/دائري/);
  });
});
