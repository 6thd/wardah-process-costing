...
export function BackupSettingsPage() {
  ...
  const exportAll = async () => {
    ...
    try {
      ...
      const date = new Date().toISOString().split('T')[0]
      download(`wardah_backup_${date}.json`, JSON.stringify(dump, null, 2), 'application/json')
      toast.success('تم تصدير نسخة كاملة من البيانات الرئيسية')
    } catch (error) {
    ...
    <div className="space-y-6">
      <div className="text-right">
        ...
        <p className="text-muted-foreground mt-2">
          تصدير بيانات مؤسستك الفعلية (JSON/CSV) — الجدولة الآلية بنية لاحقة
        </p>
      </div>
      ...
      <Card className="wardah-glass-card">
        <CardHeader className="flex flex-row items-center justify-between">
          <Button onClick={exportAll} disabled={busy !== null}>
            ...
            {busy === 'all' ? 'جارٍ التصدير…' : 'نسخة كاملة (JSON)'}
          </Button>
