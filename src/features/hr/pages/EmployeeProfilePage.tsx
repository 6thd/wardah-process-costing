...
export const EmployeeProfilePage: React.FC = () => {
  ...
  // إضافة/إلغاء بند راتب تعديل على سجل الموظف نفسه — hr.employees.update
  // هو الأقرب دلاليًا، لا مورد "salary_components" مخصص في الكتالوج الحي.
  const canUpdate = hasPermissionKey('hr.employees.update');
  ...
  const addMutation = useMutation({
    mutationFn: () => {
      if (!canUpdate) return Promise.reject(new Error(t('profile.operationFailed')));
      return upsertEmployeeSalaryComponent(id as string, {
  ...
  const removeMutation = useMutation({
    mutationFn: (componentId: string) => {
      if (!canUpdate) return Promise.reject(new Error(t('profile.operationFailed')));
      return deactivateEmployeeSalaryComponent(componentId);
    },
    onSuccess: () => {
